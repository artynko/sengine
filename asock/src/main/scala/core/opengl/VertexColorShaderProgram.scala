...
object VertexColorShaderProgram extends GLProgramMatcher {
  def matchMe(e: Entity) = e.isInstanceOf[Guid with VertexColorShader]
}

class VertexColorShaderProgram extends GLProgram {
  val cameraService = inject[CameraService]

  def bindProgram(gl: GL3): Unit = bindProgram(gl, "shaders/shader.vert", "shaders/vertexColor.frag")
